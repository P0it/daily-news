...
- `경제` (우측) — DART 공시, 경제 뉴스, 해외 증시, (Week 4+) 테마 분석 배너

**기본 진입 탭**:
- PWA 아이콘·북마크: **시사** (좌측 기본)
- 카톡 딥링크: **경제** (`?tab=economy`) — 아침 브리핑 핵심이 공시·시그널
...
**Underline 스타일 쓰지 않는 이유**: 탭이 콘텐츠 성격을 **강하게** 분리하는 역할(시사 vs 경제). Pill이 시각적 구분이 뚜렷해 사용자가 "지금 어디 있는지" 즉시 인지. Underline은 2차 필터(국내/해외)에 사용.
...
**탭 개수 상한**: 2개 유지. 3개 이상 필요하면 네비게이션 재설계 트리거 (`DECISIONS.md` 9번 재고 조건 참조).
