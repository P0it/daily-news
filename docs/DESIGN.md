...
**3-탭 체계 (DECISIONS #13 최종)**:
- `AI` (default, 좌측) — AI 모델 업데이트·툴·디자인 AI 소식 (Week 5b)
- `시사` (중앙) — 정치·사회·국제·IT 뉴스 (Google News + 공식 RSS)
- `경제` (우측) — Today's Pick 상단 섹션 + DART 공시 + 경제 뉴스 + 거시 지표 + 테마 배너

**기본 진입 탭**: **AI** (PWA 아이콘 + 카톡 딥링크 `?tab=ai`)

**2차 필터 (국내/해외)**:
- `국내` / `해외` 2 옵션 **만** (DECISIONS #13 — "전체" 제거)
- 기본값 = `국내`
- URL: `?tab=X&scope=domestic|foreign`
...
**필터 옵션 (DECISIONS #13, 2026-04-23 개정)**:

| 탭 | 옵션 | 비고 |
|----|------|------|
| AI | 국내 · 해외 | 국내 = 한국어 소스 (GeekNews 등), 해외 = 영어 소스 (Anthropic·HN 등) |
| 시사 | 국내 · 해외 | 내용 기준 분류 (국제 이슈 보도는 "해외") |
| 경제 | 국내 · 해외 | Today's Pick 도 scope 따라 국내·해외 축소 |

**"전체" 옵션 없음** — 실사용에서 카테고리 덩어리로 섞여 스캔 품질 낮음. 명시적 양자택일로 집중도 높임.
