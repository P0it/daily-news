...
SEEN_KEEP_DAYS = 14
# 로컬 브리핑·디지스트 보관 일수. 달력에서 과거 브리핑을 열람하는 범위를 정한다.
# picks_history(MAX_TRACK_DAYS=30)와 일부러 다르게 둔다 — 성과 추적(30일)과
# 아카이브 열람은 목적이 다르고, 한 달치만 남기면 축적이 드러나지 않는다.
# 브리핑 JSON 이 건당 ~100KB 라 90일이면 ~9MB 로 정적 배포에 부담이 없다.
BRIEFINGS_KEEP_DAYS = 90
