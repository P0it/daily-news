...
def cache_get(conn: Connection, task: str, input_text: str) -> str | None:
    ...
    r = conn.table("llm_cache").select("output").eq("content_hash", key).limit(1).execute()
    return r.data[0]["output"] if r.data else None


def cache_put(conn: Connection, task: str, input_text: str, output: str, model: str) -> None:
    key = hash_content(task, input_text)
