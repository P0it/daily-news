...
# 성공률이 이 아래로 떨어지면 개별 종목 실패가 아니라 Yahoo 크럼/레이트리밋 차단으로
# 보고 잠깐 쉬었다 통째로 재시도한다(발굴 유니버스 확장 후 대량 401 관측, #20).
_RETRY_SUCCESS_THRESHOLD = 0.05
_RETRY_MIN_TICKERS = 20
_RETRY_BACKOFF_SEC = 45
...
def fetch_fundamentals(tickers: list[str], scope: str) -> list[Fundamentals]:
    ...
    results = _fetch_batch(tickers, scope)

    success_rate = len(results) / len(tickers)
    if len(tickers) >= _RETRY_MIN_TICKERS and success_rate < _RETRY_SUCCESS_THRESHOLD:
        log.warning(
            "펀더멘털 수집 성공률 %.0f%% scope=%s — Yahoo 레이트리밋/크럼 차단 의심, "
            "%d초 대기 후 재시도",
            success_rate * 100,
            scope,
            _RETRY_BACKOFF_SEC,
...
def _fetch_batch(tickers: list[str], scope: str) -> list[Fundamentals]:
